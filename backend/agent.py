...
import json
import logging
from typing import Any, Optional, Callable, AsyncGenerator, List, Dict
...
# )
from tools.confluence_tools import fetch_confluence_hierarchy, fetch_confluence_pages_recursive, fetch_confluence_space_pages
...
@tool
def query_confluence_knowledge_base(query: str, space_id: Optional[str] = None) -> str:
    """
    ...
    Parameters:
      ...
      - space_id: Optional Space ID to filter the search space
    """
    emit_status(f"🔍 Searching Confluence VectorStore for: '{query}'...")
    
    # Retrieve chunks
    raw_docs = vector_service.search_confluence(query, limit=5, space_filter=space_id)
    ...
    for doc in filtered_docs:
        ...
        spid = doc.metadata.get("space_id")
        output.append(f"--- Confluence Document: '{page_title}' (ID: {pid}, Space: {spid}) ---\n{doc.page_content}\n")
...
Your job is to answer developer queries using the internal Confluence knowledge base tools.
For each query, choose the most appropriate Confluence tool to fetch documentation.
...
2. Always start by checking your vector stores using `query_confluence_knowledge_base` to retrieve relevant system specifications, setup manuals, and documents.
3. Be extremely concise and professional. Do not add fluff. Focus on service communication, dependency models, and direct answers.
4. If code is requested, show only the exact relevant code blocks or configuration snippets. Do not dump entire files.
5. If a tool execution fails, returns errors, or yields empty/incomplete results, analyze the failure, correct your parameters, and call the tool again with adjusted arguments. You are fully capable of executing tools multiple times in a loop.
6. If the query cannot be answered using the Confluence tools (e.g. details are completely missing from Confluence or the query is entirely out of scope), you MUST explicitly respond stating that you are unable to answer the query (e.g. "I am sorry, but I am unable to answer this query as the required information is not available using the provided tools."). Do not make up answers.
"""

def create_agent():
    """Compiles the agent graph using deepagents harness with Confluence tools only."""
    llm = get_llm(temperature=0.2)
    
    # Always include static tools (Jira/GitHub commented out)
    tools = [
    ...
    # Commented out GitHub and Jira integrations as they are on hold
...
async def run_agent_stream(query: str, status_cb: Callable[[str], None], history: Optional[List[Dict[str, str]]] = None) -> AsyncGenerator[str, None]:
    """
    ...
    if history:
        for msg in history:
            ...
            if role and content:
                langchain_role = "assistant" if role == "ai" else "user"
